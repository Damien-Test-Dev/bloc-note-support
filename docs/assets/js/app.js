...
(function () {
  ...
  // Hard guard: minimum nodes must exist
  ...
  // -------- View handling (defensive) --------
  function showHome() {
    if (!homeView || !editorView || !homeBtn || !editorBtn) return;
  ...
  function scheduleAutosave() {
    ...
    autosaveTimer = setTimeout(function () { autosaveIfNeeded(); }, 900);
  }
  ...
  function updateColorModeLabel(mode, tag) {
    ...
    if (mode === 'tag' && tag) colorModeEl.textContent = 'Mode : Catégorie (' + tag + ')';
    else colorModeEl.textContent = 'Mode : Personnalisée';
  }
  ...
  function renderHome(notes) {
    ...
    if (emptyStateEl) {
      if (!notes.length) emptyStateEl.hidden = false;
      else emptyStateEl.hidden = true;
    }
    ...
    for (i = 0; i < list.length; i++) {
      ...
      if (n.pinned) {
        ...
        fxDot = document.createElement('div'); fxDot.className = 'fx-dot';
        fxTop = document.createElement('div'); fxTop.className = 'fx-line top';
        fxBottom = document.createElement('div'); fxBottom.className = 'fx-line bottom';
        fxLeft = document.createElement('div'); fxLeft.className = 'fx-line left';
        fxRight = document.createElement('div'); fxRight.className = 'fx-line right';
      ...
      pin.onclick = (function (id) {
        return function (e) { e.stopPropagation(); togglePin(id); };
      })(n.id);
      ...
      del.onclick = (function (id) {
        return function (e) { e.stopPropagation(); deleteNoteById(id); };
      })(n.id);
  ...
  function renderEditor(notes, preserveEditorValue) {
    ...
    if (current) {
      ...
      if (pinBtn) pinBtn.textContent = current.pinned ? '⭐ Désépingler' : '⭐ Épingler';
      ...
      if (pinBtn) pinBtn.textContent = '⭐ Épingler';
  ...
  function saveCurrent() {
    ...
    if (dMode === 'tag' && dTag && TAG_COLORS[dTag]) dColor = hexToHsl(TAG_COLORS[dTag]);
  ...
  function copyAll() {
    ...
    if (navigator && navigator.clipboard && navigator.clipboard.writeText && secure) {
  ...
  function exportJSON() {
    ...
    var payload = { version: 2, exportedAt: nowTs(), notes: notes };
  ...
  function importJSONFile(file) {
    ...
    reader.onerror = function () { toast('Import : lecture impossible.'); };
    reader.readAsText(file);
  }

  // -------- Events (defensive bindings) --------
  if (homeBtn) homeBtn.onclick = function () { showHome(); fullRender(true); };
  if (editorBtn) editorBtn.onclick = function () { showEditor(); fullRender(true); noteEl.focus(); };

  if (searchInput) searchInput.oninput = function () { partialRender(true); };

  if (newBtn) newBtn.onclick = startNewNote;
  if (newBtnHome) newBtnHome.onclick = startNewNote;
  if (emptyCreateBtn) emptyCreateBtn.onclick = startNewNote;

  saveBtn.onclick = saveCurrent;
  clearBtn.onclick = deleteActiveOrClear;

  if (copyBtn) copyBtn.onclick = copyAll;
  if (renameBtn) renameBtn.onclick = promptRenameActive;
  ...
  if (tagSelect) tagSelect.onchange = function () { applyTag(tagSelect.value || ''); };

  if (colorPicker) colorPicker.oninput = function () { setColorFromHex(colorPicker.value, 'custom', true); };
  ...
  function applyAdvancedSliderChange() {
    ...
    var c = normalizeColor({ h: hueEl.value, s: satEl.value, l: lightEl.value });
